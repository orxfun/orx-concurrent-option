...
fn concurrent_get_or_insert_single_writer(num_readers: usize, do_sleep: bool) {
...
fn concurrent_get_or_insert_multiple_writer(
    num_writers: usize,
    num_readers: usize,
    do_sleep: bool,
) {
...
fn get_or_inserter(do_sleep: bool, maybe: &ConcurrentOption<String>) {
    for i in 0..100 {
        ...
        match i % 5 {
            1 => {
                let _ = unsafe { maybe.get_or_insert(7.to_string()) };
            }
            3 => {
                let _ = unsafe { maybe.get_or_insert(9.to_string()) };
            }
            _ => {}
        }
    }
